...
local async, await, debug, util, path, string, Future, CodeCompileError, FileSystemError, error, argument, tostring =
environment.async, environment.await, environment.debug, environment.util, environment.path, environment.string,
	environment.Future, environment.CodeCompileError, environment.FileSystemError, environment.error,
	environment.argument, environment.tostring
...
local resolve, getDirectory, getFile, getCurrentDirectory = path.resolve, path.getDirectory, path.getFile,
	...
	normalizeGamePath = function(absolutePath, gamePath)
		if not gamePath and isurl(absolutePath) then
			if absolutePath.scheme ~= "file" then
				error(FileSystemError("Cannot resolve URL '" ..
				tostring(absolutePath) .. "' because it is not a file URL."))
			end
		...
		elseif firstByte == 0x2E then
			...
			if secondByte == 0x2F or (secondByte == 0x2E and byte(absolutePath, 3) == 0x2F) then
				...
				if currentDir == "" then
					error(FileSystemError("Cannot resolve relative path '" ..
					tostring(absolutePath) .. "' because main file is unknown."))
				end
...
do
	...
	lib.Exists = function(filePath, gamePath, skipNormalize)
		...
		return (isFileMounted(filePath, gamePath, true) or isDirMounted(filePath, gamePath, true)) or
		Exists(filePath, gamePath) or (CLIENT and luaGamePaths[gamePath] and Exists("lua/" .. filePath, "WORKSHOP"))
	end
	isDir = function(filePath, gamePath, skipNormalize)
		if not skipNormalize then
			filePath, gamePath = normalizeGamePath(filePath, gamePath)
		end
		return isDirMounted(filePath, gamePath, true) or IsDir(filePath, gamePath) or
		(CLIENT and luaGamePaths[gamePath] and IsDir("lua/" .. filePath, "WORKSHOP"))
	end
	lib.IsDir = isDir
...
do
	...
	do
		...
		local downloadUGC = _G.steamworks and _G.steamworks.DownloadUGC or nil
		if downloadUGC then
			environment.steamworks.DownloadUGC = function(workshopID)
				argument(workshopID, 1, "string")
				local fut = Future()
				downloadUGC(workshopID, function(filePath, fileObject)
					if filePath then
						if fileObject then
							setmetatable(fileObject, base)
						end
						fut:setResult({
							path = filePath,
							file = fileObject
					...
					fut:setError("Steam workshop download failed.")
					return nil
				end)
				return fut
			end
	...
	do
		local ReadULong, WriteULong, ReadByte, Seek, Size, Skip, Tell = legacy.ReadULong, legacy.WriteULong,
			legacy.ReadByte, legacy.Seek, legacy.Size, legacy.Skip, legacy.Tell
...
local asyncRead
do
	local captureStack, appendStack, popCallStack, pushCallStack = FileSystemError.captureStack,
		FileSystemError.appendStack, FileSystemError.popCallStack, FileSystemError.pushCallStack
	local functions = {
		{
			"Append",
			false
				...
				if iState == 0 then
					if verifyHash then
						...
						if not (cache and (not cache.hash or cache.hash == SHA256(iContent)) and cache.size == len(iContent)) then
							fut:setError(FileSystemError("File hash mismatch for '" ..
							absoluteGamePath(filePath, gamePath, false) .. "'"))
							return nil
						end
					end
...
do
	...
	environment.loadfile = function(filePath, mode, env, config, gamePath, skipNormalize, verifyHash)
		...
		if CLIENT and luaGamePaths[gamePath] and not isFileMounted(filePath, gamePath, true) and getExtension(filePath, false) == "lua" then
			local ok, result = pcall(CompileFile, filePath)
			if not ok then
				error(CodeCompileError("Failed to compile '" ..
				absoluteGamePath(filePath, gamePath, true) .. "', " .. result))
			end
			if not result then
		...
		return load(read(filePath, gamePath, nil, true, verifyHash), absoluteGamePath(filePath, gamePath, true), mode,
			env, config)
	end
end
do
